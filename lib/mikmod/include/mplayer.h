...
typedef struct MPLAYER
{
    ...
    uint       *pos_played;     // Indicates if the position has been played.
...
extern BOOL     Player_Paused(MPLAYER *ps);

extern void     Player_SetVolume(MPLAYER *mp, int volume);
extern void     Player_VolumeFade(MPLAYER *ps, int start, int dest, int speed);
...
extern uint     Player_HandleTick(MD_VOICESET *vs, MPLAYER *playerinfo);
