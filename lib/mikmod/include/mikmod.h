...
#define DMODE_SURROUND     1024         // Enable support for dolby surround (inverse waveforms left/right)
...
//    MD_STREAM *stream;       // streaming information
    ...
    int    residx;           // resource index (temp hack)
} MD_SAMPLE;
...
// MD_SAMPLE Stuffholio - for playing sound effects
// =================================================

extern MD_SAMPLE *mdsfx_create(MDRIVER *md);
extern MD_SAMPLE *mdsfx_duplicate(MD_SAMPLE *src);
extern void       mdsfx_free(MD_SAMPLE *samp);

extern void       mdsfx_play(MD_SAMPLE *s, MD_VOICESET *vs, uint voice, int start);
extern int        mdsfx_playeffect(MD_SAMPLE *s, MD_VOICESET *vs, uint start, uint flags);


// Voiceset crapola -- from -- VOICESET.C --
// =========================================

extern MD_VOICESET *Voiceset_Create(MDRIVER *drv, MD_VOICESET *owner, uint voices, uint flags);
extern void         Voiceset_SetPlayer(MD_VOICESET *vs, uint (*player)(struct MD_VOICESET *voiceset, void *playerinfo), void *playerinfo);
extern MD_VOICESET *Voiceset_CreatePlayer(MDRIVER *md, MD_VOICESET *owner, uint voices, uint (*player)(struct MD_VOICESET *voiceset, void *playerinfo), void *playerinfo, BOOL hardware);
...
extern int      Voice_Find(MD_VOICESET *vs, uint flags);
