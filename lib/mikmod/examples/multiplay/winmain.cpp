...
BOOL CALLBACK DlgProc(HWND hDlg,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		device = Mikmod_Init(44100, 200, NULL, MD_STEREO, CPU_NONE, DMODE_16BITS | DMODE_INTERP | DMODE_SAMPLE_DYNAMIC | DMODE_NOCLICK);
		if(device==NULL)
		{
	...
	case WM_DESTROY:
        ...
        if(mp1) Player_FreeSong(mp1);
        if(mf1) Unimod_Free(mf1);

        if(mp2) Player_FreeSong(mp2);
        if(mf2) Unimod_Free(mf2);

        if(s1) unisample_free(s1);
        if(s2) unisample_free(s2);
...
BOOL HandleCommand(HWND hDlg,DWORD id)
{
	...
	case W1PLAY:
		v1=unisample_sndfx(s1,wvs,0,0);
		break;
	case W2PLAY:
		v2=unisample_sndfx(s2,wvs,0,0);
		break;
	...
	case W2BROWSE:
		...
		{
			ofn.lpstrFilter="All modules (.it; .xm; .s3m; .mod; .stm; .m15)\0*.it; *.xm; *.s3m; *.mod; *.stm; *.m15\0All files (*.*)\0*.*\0";
		...
		if(GetOpenFileName(&ofn)==true)
		{
			...
			case W1BROWSE:
				...
				if(s1) unisample_free(s1);
				s1=WAV_LoadFN(device, szFilename);
				break;
			case W2BROWSE:
				SetDlgItemText(hDlg,W2FILE,szFilename);
				if(s2) unisample_free(s2);
				s2=WAV_LoadFN(device, szFilename);
