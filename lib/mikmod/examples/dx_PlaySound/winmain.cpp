...
{
    return !device->device.VoiceStopped(v);
}
...
VOID OnOpenSoundFile( HWND hDlg ) 
{
    ...
    if( TRUE == GetOpenFileName( &ofn ) )
    {
		...
		if(s) unisample_free(s);
		s=WAV_LoadFN(device, strFileName);
