...

{
    ...
    {   voices += num;
        if(mixmode == MM_STATIC)
        {   md->vmh = (MD_VOICESET **)_mm_realloc(md->vmh, sizeof(MD_VOICESET *) * voices);
            //memset(&md->vmh[md->hardvoices], num*sizeof(MD_VOICESET *), 0);
            md->hardvoices = voices;
            md->device.SetHardVoices(voices);
        } else
        {   md->vms = (MD_VOICESET **)_mm_realloc(md->vms, sizeof(MD_VOICESET *) * voices);
            //memset(&md->vms[md->softvoices], num*sizeof(MD_VOICESET *), 0);
            for(i=md->softvoices; i<voices; i++)
                md->vms[i] = NULL;
            md->softvoices = voices;
            md->device.SetSoftVoices(voices);
        }
    }
    return num;
}
...
// for the mixing mode are deallocated.
{
    ...
    {   if(md->hardvoices <= num)
        {   md->device.SetHardVoices(md->hardvoices = 0);
            _mm_free(md->vmh, "Voice management tables (hardware)");
        } else
        {   md->hardvoices -= num;
            md->device.SetHardVoices(md->hardvoices);
    ...
    {   if(md->softvoices <= num)
        {   md->device.SetSoftVoices(md->softvoices = 0);
            _mm_free(md->vms, "Voice management tables (software)");
        } else
        {   md->softvoices -= num;
            md->device.SetSoftVoices(md->softvoices);
            // See above.. :)
        }
    }
}
...
// =====================================================================================
// Creates a new voice set.  Makes the voiceset the child of the
// given 'owner.'  The voiceset has no parent if owner is null.

// Returns a handle to the new voice set.
// returns -1 on error!
{
    MD_VOICESET *cruise, *vs;

    vs = _mm_calloc(1, sizeof(MD_VOICESET));

    /*if(voices)
    {   if((vs->vdesc = (MD_VDESC *)_mm_calloc(voices, sizeof(MD_VDESC))) == NULL)
        {   _mm_free(vs);   return NULL;   }
    }*/
...
// =====================================================================================
{
    ...
    _mmlogd("Voiceset > Deinitializing...");

    if(vs->vdesc)
    {   MD_VOICESET **cruise = (vs->flags & MDVS_STATIC) ? vs->md->vmh : vs->md->vms;
        
        _mmlogd(" > Unloading voiceset descriptor array");
        
        for(i=0; i<vs->voices; i++)
        {   vs->md->device.VoiceStop(vs->vdesc[i].voice);
            cruise[vs->vdesc[i].voice] = NULL;
        }

        _mm_free(vs->vdesc, NULL);
    }
    ...
    {   i = md->hardvoices;
        while(i) if(vs->md->vmh[--i]) break;
        if((md->hardvoices - i) > 5) freevoices(md, MM_STATIC, md->hardvoices - i);
    } else
    {   //i = md->softvoices;
        //while(i) if(vs->md->vms[--i]) break;
        //if((md->softvoices - i) > 10) freevoices(md, MM_DYNAMIC, md->softvoices - i);
    }

    _mm_free(vs, "Voiceset handle");
}

// =====================================================================================
    int Voiceset_SetNumVoices(MD_VOICESET *vs, uint voices)
// =====================================================================================
// Set the number of voices allocated to an audio type.  Provided mainly for use
// by the player only,and is generally a good idea to avoid using this function
// unless the voiceset in question is not in use currently.  Increasing the
// number of voices is usually fine (tho may not always succeed).

// Note: Decreasing the number of voices simply strips out the voices from
// the end of the set's voice list.  In the future, I may make it smart:
...
//  c) rest are deallocated from the back
{
    ...
    if(voices > vs->voices)
    {
        vs->vdesc = (MD_VDESC *)_mm_realloc(vs->vdesc, voices * sizeof(MD_VDESC));
        memset(&vs->vdesc[vs->voices], 0, (voices-vs->voices) * sizeof(MD_VDESC));
            
        // -- Allocate voices to this voiceset --

        // Unfortunately this is a somewhat complext procedure.  We have to determine if we
...
// =====================================================================================
// Stops the mixer in its tracks for the voices within this set.  No mixing
// or updates of the voices will occur, leaving the voices in the exact same
// state until resumed with Voiceset_EnableOutput.
{
    uint i;
    ...
    {   // Have the driver stop updating all voices.  Note, however, that the
        ...
        if(!vs->md->device.VoiceStopped(vs->vdesc[i].voice))
...
// =====================================================================================
{
    ...
    for(i=0; i<vs->voices; i++)
        ...
        {   vs->md->device.VoiceResume(vs->vdesc[i].voice);
            vs->vdesc[i].flags &= ~MDVD_PAUSED;
        }
...
// =====================================================================================
    static void MD_UpdateVoiceVolume(MD_VOICESET *vs, uint voice)
// =====================================================================================
{
    MMVOLUME   q;
    ...
    //if(md->mode & DMODE_REVERSE) pan = -pan;
    //pan = (pan * md->pansep) / 128;

    q.front.left = ((vs->absvol * vs->vdesc[voice].volume) * (PAN_RIGHT - vs->vdesc[voice].pan_lr) * (PAN_REAR - vs->vdesc[voice].pan_fb)) / (128*128);
    q.front.right = ((vs->absvol * vs->vdesc[voice].volume) * (PAN_RIGHT + vs->vdesc[voice].pan_lr) * (PAN_REAR - vs->vdesc[voice].pan_fb)) / (128*128);
    //q.rear.left  = ((vs->absvol * vs->vdesc[voice].volume) * (PAN_RIGHT - vs->vdesc[voice].pan_lr) * (PAN_REAR + vs->vdesc[voice].pan_fb)) / (128*128);
    //q.rear.right  = ((vs->absvol * vs->vdesc[voice].volume) * (PAN_RIGHT + vs->vdesc[voice].pan_lr) * (PAN_REAR + vs->vdesc[voice].pan_fb)) / (128*128);
    ...
    vs->md->device.VoiceSetVolume(vs->vdesc[voice].voice, &q);
}

// =====================================================================================
    void Voice_SetStream(MD_VOICESET *vs, uint voice, MD_STREAM *stream, uint format)
// =====================================================================================
// setup a voice for streaming audio based on the give streaming information and
// sample format.  A block of memory is allocated from the driver to allow the
// stream to function, and the stream data is attached to the voiceset's voice.
...
// =====================================================================================
{
    ...
    vs->md->device.VoiceSetFrequency(vs->vdesc[voice].voice, frq);
}
...
{
    vs->md->device.VoiceResume(vs->vdesc[voice].voice);
    vs->vdesc[voice].flags &= ~MDVD_PAUSED;
}

void Voice_Play(MD_VOICESET *vs, uint voice, int handle, int start, int length, int reppos, int repend, int suspos, int susend, uint flags)

// Signals to the driver to start playing a voice.
// if the loopstart vaules are illegal (<0 or >length), or handle is illegal,
// the loop will not be played.
{
    if(VoiceCheck() || (handle<0) || (length<=0)) return;

    if(repend > length) repend = length;    // repend can't be bigger than size
    if(susend > length) susend = length;

    if(start == SF_START_CURRENT)
        start = vs->md->device.VoiceGetPosition(vs->vdesc[voice].voice);
    if(start == SF_START_BEGIN)
        start = flags & SL_REVERSE ? length : 0;
    else if(start < 0) start = 0;

    vs->md->device.VoicePlay(vs->vdesc[voice].voice, handle, start, length, reppos, repend, suspos, susend, flags);
}

void Voice_ReleaseSustain(MD_VOICESET *vs, uint voice)
{
    if(VoiceCheck()) return;
    vs->md->device.VoiceReleaseSustain(vs->vdesc[voice].voice);
}

void Voice_Pause(MD_VOICESET *vs, uint voice)
{
    if(VoiceCheck()) return;
    vs->md->device.VoiceStop(vs->vdesc[voice].voice);
    vs->vdesc[voice].flags |= MDVD_PAUSED;
}
...
BOOL Voice_Stopped(MD_VOICESET *vs, uint voice)
{
    ...
    return(vs->md->device.VoiceStopped(vs->vdesc[voice].voice));
}
...
{
    if(VoiceCheck()) return 0;
    return(vs->md->device.VoiceGetPosition(vs->vdesc[voice].voice));
}
...
{
    if(VoiceCheck()) return 0;
    return(vs->md->device.VoiceRealVolume(vs->vdesc[voice].voice));
}
...
int Voice_Find(MD_VOICESET *vs, uint flags)
{
    ...
    // So do a wghile loop to find first non-critical voice.
...
static void child_volupd(MD_VOICESET *vs, int effective_volume)
{
    ...
    if(vs->absvol == (effective_volume / 0x10000)) return;
    
    vs->absvol = effective_volume / 0x10000;

	// set my own voices
	for(i=0; i<vs->voices; i++) MD_UpdateVoiceVolume(vs, i);
