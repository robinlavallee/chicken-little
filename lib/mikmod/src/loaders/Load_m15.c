...
{
    _mm_free(handle, "15-inst loader handle");
}
...
static void M15_ConvertNote(MODNOTE *n, BOOL ust_loader)
{
    ...
    {   switch(effect)
        {   case 0:  break;
            case 1:
                pt_write_effect(0,effdat);
            break;

            case 2:  
                if(effdat&0xf) pt_write_effect(1,effdat&0xf);
                if(effdat>>2)  pt_write_effect(2,effdat>>2);
            break;
    ...
    } else pt_write_effect(effect,effdat);
}
...
//  3 byte format.
{
    ...
    {   // Load the pattern into the temp buffer
        ...
        {   utrk_settrack(s & 3);
            M15_ConvertNote(patdat, ust_loader);
            if((s & 3) == 3) utrk_newline();   // newline every 4 channels!
        }
...
BOOL M15_Load(MODULEHEADER *mh, UNIMOD *of, MMSTREAM *mmfile)
{
    ...
    if((mh->patdat = _mm_calloc(of->numpat*64*4, sizeof(MODNOTE))) == NULL) return 0;
    ...
    if(ust_loader)
        of->modtype = _mm_strdup("Ultimate Soundtracker");
    else
        of->modtype = _mm_strdup("Soundtracker");
    ...
    {   // convert the samplename
        ...
        if((q->length > 0x7ffff) || (q->loopstart > q->length))
...
CHAR *M15_LoadTitle(MMSTREAM *mmfile)
{
   ...
   if(!fread(s,22,1,mmfile->fp)) return NULL;
