...
void *MTM_Init(void)
{
    return _mm_calloc(1,sizeof(MTM_HANDLE));
}
...
void MTM_Cleanup(MTM_HANDLE *handle)
{
    ...
    {   if(handle->globtrack)
        {
            _mmlogd(" > Freeing MTM loader globaltrack temps");
            for(t=0; t<handle->numtrk; t++) _mm_free(handle->globtrack[t], NULL);
            _mm_free(handle->globtrack, NULL);
        }
        _mm_free(handle, "MTM loader handle");
    }
}
...
BOOL MTM_Load(MTM_HANDLE *h, UNIMOD *of, MMSTREAM *mmfile)
{
    ...
    of->initspeed = 6;
    of->inittempo = 125;
    of->modtype   = _mm_strdup(MTM_Version);
    of->numchn    = h->mh.numchannels;
    of->numtrk    = h->mh.numtracks+1;         // unique (or maybe not) tracks in song
    of->songname  = DupStr(h->mh.songname,20); // make a cstr of songname
    ...
    utrk_init(1);
    if(!(h->globtrack = (UBYTE **)_mm_calloc(h->numtrk = of->numtrk, sizeof(UBYTE *)))) return 0;
    ...
    {   int          s;
        UBYTE        a,b,c;
        unsigned int inst,note,eff,dat;
        ...
        {   a = _mm_read_UBYTE(mmfile);
            ...
            utrk_write_inst(inst);
            if(note!=0)  utrk_write_note(note+25);
        ...
        of->tracks[t]   = utrk_dup_track(0);
        h->globtrack[t] = utrk_dup_global();
    }
    ...
    // read comment field
    if(h->mh.commentsize)
    {   if(!(of->comment=(CHAR *)_mm_malloc(h->mh.commentsize+1))) return 0;
