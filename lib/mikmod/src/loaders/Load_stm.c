...
void *STM_Init(void)
{
    return _mm_calloc(1,sizeof(STMHEADER));
}

void STM_Cleanup(void *handle)
{
    _mm_free(handle, "STM loader handle");
}
...
void STM_ConvertNote(STMNOTE *n)
{
  ...
  if(note==254 || note==252) pt_write_effect(0xc,0); // <- note off command (???)
     else
  // if note < 251, then all three bytes are stored in the file
  if(note<251) utrk_write_note((((note>>4)+2)*12)+(note&0xf) + 1);      // <- normal note and up the octave by two

    if(vol<65) pt_write_effect(0xc,vol);

    if(cmd!=255)
    {   if(cmd==1)
        {   pt_write_effect(0xf,inf>>4);
        } else S3MIT_ProcessCmd(NULL, cmd, inf, 3,PTMEM_PORTAMENTO);
    }
}
...
BOOL STM_Load(STMHEADER *mh, UNIMOD *of, MMSTREAM *mmfile)
{
    ...
    of->modtype   = _mm_strdup(STM_Version);
    ...
    {   // load sample info
        ...
        if((mh->sample[t].loopend>0) && (mh->sample[t].loopend != 0xffff)) q->flags |= SL_LOOP;
