...
{
    return md->device.FreeSampleSpace(type);
}


ULONG MD_SampleLength(MDRIVER *md, int type, SAMPLOAD *s)
{
    return md->device.RealSampleLength(type, s);
}
...
// Generally, this function is only called from SL_LoadSamples.
{
    ...
    SL_Init(s);
    result = md->device.SampleLoad(s, type);
    SL_Exit(s);
...
{
    if(handle >= 0) md->device.SampleUnLoad(handle);
}
...
//    uint  dummy;
    ...
    md = (MDRIVER *)_mm_malloc(sizeof(MDRIVER));
    ...
    if(md->device.Init(latency, optstr))
    {   Mikmod_Exit(md);
    ...
    md->device.SetMode(mixspeed, drvmode, chanmode, cpumode);

    //initialized  = 1;

    md->name = md->device.Name;

    md->device.GetMode(&md->mixspeed, &md->mode, &md->channels, &md->cpu);
...
void Mikmod_Exit(MDRIVER *md)
{
    ...
    // Free up all allocated voice sets--
...
BOOL Mikmod_SetMode(MDRIVER *md, uint mixspeed, uint mode, uint channels, uint cpumode)
{
    ...
    ok = md->device.SetMode(mixspeed, mode, channels, cpumode);
...
void Mikmod_GetMode(MDRIVER *md, uint *mixspeed, uint *mode, uint *channels, uint *cpumode)
{
    ...
    md->device.GetMode(mixspeed ? mixspeed : &dummy, mode ? mode : &dummy,
...
{
    return(md->device.GetActiveVoices());
}
