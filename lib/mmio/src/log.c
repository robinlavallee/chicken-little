...
int log_init(const CHAR *logfile, BOOL val)
{
    ...
    if((str = calloc(512, sizeof(CHAR))) == NULL) return 1;
...
// Func: printlogv
// Direct replacement for printf using logging features.  Anything logged
// using this function will ALWAYS be displayed to the console (useful for
// critical errors and such).

void log_printv(const CHAR *fmt, ... )
{
    static va_list argptr;
    ULONG cnt=0;
