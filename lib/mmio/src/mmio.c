...
// Creates an MMSTREAM structure from the given file pointer and seekposition
{
    ...
    mmf = (MMSTREAM *)malloc(sizeof(MMSTREAM));
...
// ===========================================================================
void StringWrite(CHAR *s, MMSTREAM *fp)
// ===========================================================================
...
// ===========================================================================
{
    ...
    mfp         = _mm_calloc(1,sizeof(MMSTREAM));
    mfp->fp     = fp;
    mfp->iobase = 0;
    mfp->dp     = NULL;
...
// ===========================================================================
{
    ...
    if(stream->fp)
    {   // file mode...
        return fseek(stream->fp,(whence==SEEK_SET) ? offset+stream->iobase : offset, whence);
    } else
    {   long   tpos;
        switch(whence)
...
{
   if(!stream) return 0;
   return (stream->fp) ? (ftell(stream->fp) - stream->iobase) : stream->seekpos;
}
...
// file.  Copy begins whereever the current read pointers are for the given
// files.  Returns 0 on error.
{
    ULONG todo;
...
// ===========================================================================
void _mm_write_string(CHAR *data, MMSTREAM *fp)
// ===========================================================================
{
...
#define filewrite_M_SWORD(x,fp) filewrite_SBYTE((UBYTE)x>>8,fp); filewrite_UBYTE(x,fp)
#define filewrite_M_UWORD(x,fp) filewrite_UBYTE((UBYTE)x>>8,fp); filewrite_UBYTE(x,fp)
#define datawrite_M_SWORD(x,fp) datawrite_SBYTE((UBYTE)x>>8,fp); datawrite_UBYTE(x,fp)
#define datawrite_M_UWORD(x,fp) datawrite_UBYTE((UBYTE)x>>8,fp); datawrite_UBYTE(x,fp)
#define filewrite_M_SLONG(x,fp) filewrite_M_SWORD((UWORD)x>>16,fp); filewrite_M_UWORD(x,fp)
#define filewrite_M_ULONG(x,fp) filewrite_M_UWORD((UWORD)x>>16,fp); filewrite_M_UWORD(x,fp)
#define datawrite_M_SLONG(x,fp) datawrite_M_SWORD((UWORD)x>>16,fp); datawrite_M_UWORD(x,fp)
#define datawrite_M_ULONG(x,fp) datawrite_M_UWORD((UWORD)x>>16,fp); datawrite_M_UWORD(x,fp)
...
#ifdef MM_BIG_ENDIAN
#define filewrite_M_SWORDS(x,y,z) fwrite((void *)x,1,y*sizeof(SWORD),z)
#define filewrite_M_UWORDS(x,y,z) fwrite((void *)x,1,y*sizeof(UWORD),z)
#define filewrite_M_SLONGS(x,y,z) fwrite((void *)x,1,y*sizeof(SLONG),z)
#define filewrite_M_ULONGS(x,y,z) fwrite((void *)x,1,y*sizeof(ULONG),z)
...
void                                                             \
_mm_write_##type_name##S (type *data, int number, MMSTREAM *fp)  \
{                                                                \
    if(fp->fp)                                                   \
    {   while(number>0)                                          \
...
{
    if(fp->fp)
    {   fread(buffer,sizeof(CHAR),number,fp->fp);
        return !feof(fp->fp);
    } else
