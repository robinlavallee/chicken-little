...
//
//
//	Jul 22, 2000	Converted to 'C' code (Nathan Youngman)
//-----------------------------------------------------------------------------
#include <windows.h>
#include <windowsx.h>
#include <ddraw.h>
...
DWORD App_GetDXVersion(void)
{
    ...
    if( FAILED( IDirectDraw_QueryInterface(pSurf, &IID_IDirectDrawSurface3,
                                       (VOID**)&pSurf3 ) ) )
    {
        IDirectDraw_Release(pDDraw);
        FreeLibrary( hDDrawDLL );
    ...
    if( FAILED( IDirectDraw_QueryInterface(pSurf, &IID_IDirectDrawSurface4,
                                       (VOID**)&pSurf4 ) ) )
    {
        IDirectDraw_Release(pDDraw);
        FreeLibrary( hDDrawDLL );
